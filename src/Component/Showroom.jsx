...
const Showroom = ({ showrooms, banShowroom, Activeshowroom }) => (
            ...
            {showrooms.map((showroom) => (
                <div key={showroom.id} className="bg-white p-6 rounded-lg shadow-md hover:shadow-lg transition sm:flex justify-between items-center">
                    ...
                    {showroom.status !== "Active" && (
                        <button
                        onClick={() => Activeshowroom (showroom.id)}
                        className="bg-green-500 text-white px-4 sm:py-2 rounded-lg hover:bg-green-600 transition duration-200"
                    >
                         Active
                    </button>
                    )
